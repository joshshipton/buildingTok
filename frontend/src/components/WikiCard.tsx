        ...
        const fetchArticleContent = async () => {
            try {
                ...
                const response = await fetch(
                    `${BASE_API}action=query&format=json&origin=*&prop=extracts|categories&` +
                    `pageids=${article.pageid}&explaintext=1&exintro=1&` +
                    `exsentences=5&cllimit=500`  // Get all categories to properly check
                );
                const data = await response.json();
                const page = data.query.pages[article.pageid];
    
                // Check if the article is in the Architecture category
                const isArchitecture = page.categories?.some(
                    (cat: any) => cat.title === 'Category:Architecture'
                );
    
                if (isArchitecture && page.extract) {
                    setArticleContent(page.extract);
                } else {
                    setArticleContent("This article is not related to architecture.");
                }
            } catch (error) {
                console.error('Error fetching article content:', error);
                setArticleContent("something went wrong, whoops. blame josh");
            }
        };
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: article.title,
                    text: articleContent || '',
                    url: `${WIKI_BASE_URL}${article.pageid}`
                });
            } catch (error) {
